...
func feedback(first_d, second_d, feedback, change_scene_on_feedback=false):
	...
	if change_scene_on_feedback:
		call_deferred("change_scene", "res://World/room/orphanage_office.tscn")
...
func _on_foreach_pressed():
	enemy_dialogue("Did you already forget it?", "You must listen whenever I teach you!", "I didn't teach you about this magic!")

# Handle the action when the inbox button is pressed
func _on_inbox_pressed():
	enemy_dialogue("Nope, ListKey magic is not an E-mail or mailbox", "Let me give you a hint", "Its count usually starts from ZERO, Remember that.")
...
func _on_incode_pressed():
	enemy_dialogue("Wrong Valen!", "It sounds magical but it was not the right answer", "It is a locator that helps you quickly find and access items in ListKey Magic")
